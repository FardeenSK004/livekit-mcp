...
## Error Handling
- Missing token: `401 Unauthorized` with `{"error": "unauthorized", "error_description": "Authorization token required"}`
- Invalid/Expired token: `401 Unauthorized` with `{"error": "invalid_token", "error_description": "Token is invalid or expired"}`
- Unprotected routes: `/health`, `/docs`, `/` bypass authentication.
